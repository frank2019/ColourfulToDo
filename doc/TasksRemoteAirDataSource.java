...
import com.colour.time.todo.NetRequest.Toolsbox;
...
public class TasksRemoteAirDataSource implements TasksDataSource {
    ...
    private static void addTask(String title, String description) {
        ...
        Toolsbox.getInstance().addTasks(tasksRequestVo,null);
    ...
    public void getTasks(final @NonNull LoadTasksCallback callback) {
        ...
        //yyyy-MM-dd HH:mm:ss
        try {
            start = Toolsbox.dateToStamp("2018-01-01 00:00:00");
        ...
        Toolsbox.getInstance().getTasks(queryRequestVo, new Callback<TasksResponseVo>() {
            @Override
            public void onResponse(Call<TasksResponseVo> call, Response<TasksResponseVo> response) {
                ...
                if(taskRemotes == null || taskRemotes.length == 0){

                }else {
                    for(int i = 0; i< taskRemotes.length; i++){
                        tasks.add(new Task(taskRemotes[i].getTitle(), taskRemotes[i].getDescription(), taskRemotes[i].getCompleted()));
                    }
                }
